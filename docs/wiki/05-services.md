...
| [recurring-service.ts](file:///workspace/packages/shared/src/services/recurring-service.ts) | 经常性交易模板补单引擎 | 54 | 是（间接） | 是（间接） |
...
**关键细节**（[recurring-service.ts:22-51](file:///workspace/packages/shared/src/services/recurring-service.ts#L22-L51)）：
...
- **循环后收尾**（[recurring-service.ts:44-50](file:///workspace/packages/shared/src/services/recurring-service.ts#L44-L50)）：若 `next_due_date` 已推进，再次调用 `updateRecurring` 持久化新到期日；若新到期日超过 `end_date`，同时置 `is_active = 0`
